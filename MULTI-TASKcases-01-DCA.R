...
path = "C:/Users/zuoya/Documents/FYC-DCA/TRAIN_EGFR_information.csv"
tabular_image_file <- read.csv(file = path,header = TRUE, sep=",")
colnames(tabular_image_file)
#For Loop Example

# countries <- c('India', 'U K', 'Japan', 'U S A', 'Australia', 'China')
countries <- c("DL_radiomics_clinic","DL","Radiomics")

for (str in countries) {
  print(paste("Countries are:  ", str))
}
print("----This is Coming from Outside the For Loop---")
dca <- function(data, outcome, predictors, xstart=0.10, xstop=0.99, xby=0.01,
...
countries <- c("DL_radiomics_clinic","DL","Radiomics")  #01
...
#   dev.off()
# }
png("C:/Users/zuoya/Documents/FYC-DCA/TRAIN-DCA.png", width = 225, height = 225, units='mm', res = 1000)
# pdf("dca-external1_1921-2023.pdf")
dca(data=tabular_image_file, outcome="label", predictors=c("DL_radiomics_clinic","DL","Radiomics"), smooth="TRUE")
